...
packages <- c("matrixNormal","mniw","MCMCpack","mvtnorm","sna","MixMatrix","MASS","GIGrvg")
ipak(packages)
...
#########################################
#########################################
MFM_MxN_equal_cov_sparse<- function(data, niterations, alpha, beta, psi, rho, GAMMA,
                              M0, Sigma0, Omega0, initNClusters, MLE.initial=FALSE)
{
  ## Model: Y_{i}|z,theta \sim MN(theta_{z_i}) ##
  ##        M_{r} \sim MN(M0, Sigma0, Omega0), r = 1,...,k ##
  ...
  #######################
  #Priors from Weining
  phi_mat_U<-matrix(1,p,p)
  phi_mat_V<-matrix(1,q,q)

  psi_mat_U<-matrix(1,p,p)
  psi_mat_V<-matrix(1,q,q)

  rho_mat_U<-matrix(0.5,p,p)
  rho_mat_V<-matrix(0.5,q,q)

  #Corresponds to V matrix in paper
  #W_mat_U<-sqrt((rho_mat_U*tausqU)/(1-rho_mat_U))
  #W_mat_V<-sqrt((rho_mat_V * tausqV)/(1-rho_mat_V))
  W_mat_U<-matrix(tausqU,p,p)
  W_mat_V<-matrix(tausqV,q,q)
  ...
  for (iter in 1:niterations)
  {
    ...
    centered_data<-array(data = NA, dim = dim(data))
    for(i in 1:dim(data)[3]){
      centered_data[,,i]<-data[,,i]-M[,,clusterAssign[i]]
    }

    #New U Update
    updateU<-U_update(U =U,data = centered_data,solveU = solve_U,solveV = solve_V,rho_mat_U = rho_mat_U, psi_mat_U = psi_mat_U,
                      phi_mat_U = phi_mat_U,W_mat_U = W_mat_U)

    phi_mat_U<-updateU$phi_mat_U
    rho_mat_U<-updateU$rho_mat_U
    psi_mat_U<-updateU$psi_mat_U
    solveU<-updateU$solveU
    W_mat_U<-updateU$W_mat_U
    U<-updateU$U
    ...
    updateV<-V_update(V = V, data = centered_data,solveU = solve_U, solveV = solve_V,
                      rho_mat_V = rho_mat_V,phi_mat_V = phi_mat_V, W_mat_V = W_mat_V,psi_mat_V = psi_mat_V)
    phi_mat_V<-updateV$phi_mat_V
    rho_mat_V<-updateV$rho_mat_V
    psi_mat_V<-updateV$psi_mat_V
    solveV<-updateV$solveV
    W_mat_V<-updateV$W_mat_V
    V<-updateV$V
...
MFM_MxN_equal_cov_sparse_newprior<- function(data, niterations, alpha, beta, psi, rho, GAMMA,
                                    M0, Sigma0, Omega0, initNClusters, MLE.initial=FALSE)
{
  ## Model: Y_{i}|z,theta \sim MN(theta_{z_i}) ##
  ##        M_{r} \sim MN(M0, Sigma0, Omega0), r = 1,...,k ##
  ...
  if(MLE.initial){
    MLE_data <- MixMatrix::MLmatrixnorm(data)
    MLD_data<-MixMatrix::MLmatrixnorm(data[,,1:10])
    M <- array(0, dim=c(p,q,initNClusters))
    for(r in 1:initNClusters){
      M[,,r] <- MLE_data$mean
    }
